...
float windspeedkmh; // [mph instantaneous wind speed]
float windgustkmh; // [mph current wind gust, using software specific time period]
int windgustdir; // [0-360 using software specific time period]
float windspdkmh_avg2m; // [mph 2 minute average wind speed mph]
...
float humidity; // [%]
float tempc; // [temperature F]
float rainmm; // [rain inches over the past hour)] -- the accumulated rainfall in the past 60 min
volatile float dailyrainmm; // [rain inches so far today in local time]
...
// Activated by the magnet and reed switch in the rain gauge, attached to input D2
{
  ...
  {
    dailyrainmm += 0.2794; //Each dump is 0.2794 mm of water
...
void loop()
{
  ...
  if(millis() - lastSecond >= 1000)
  {
    ...
    windspdavg[seconds_2m] = (int)windspeedkmh;
...
void midnightReset()
{
  dailyrainmm = 0; //Reset daily amount of rain

  windgustkmh = 0; //Zero out the windgust for the day
